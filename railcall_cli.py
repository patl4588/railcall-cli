...
def _install_local_action_set():
    ...
    Returns (action_ids, providers, configured_providers, studio_ok).
    studio_ok=False when Studio isn't running — the caller uses it to
    soften the dep-check output ('start Studio to enable this check')
    rather than falsely reporting everything as missing.
    configured_providers = the subset of `providers` for which the
    operator has vault credentials so the caller can flag
    provider-referenced-but-no-creds as a distinct warning class."""
    port = os.environ.get("STUDIO_PORT", "8799")
    base = f"http://127.0.0.1:{port}"
    try:
        with urllib.request.urlopen(base + "/api/actions", timeout=3) as r:
            actions_data = json.loads(r.read().decode("utf-8"))
    except Exception:
        return set(), set(), set(), False
    ids, provs = set(), set()
    for a in (actions_data.get("actions") or []) + (actions_data.get("module_actions") or []):
        if isinstance(a.get("id"), str):
            ids.add(a["id"])
        if isinstance(a.get("provider"), str):
            provs.add(a["provider"])
    configured = _install_fetch_configured_providers(base)
    return ids, provs, configured, True


def _install_fetch_configured_providers(base_url):
    """Set of provider ids the operator has configured credentials for.
    Returns empty set on any fetch failure — a missing configured set
    won't produce false 'missing credentials' warnings because the
    caller only flags provider references where the action IS available
    (which requires studio_ok=True)."""
    try:
        with urllib.request.urlopen(base_url + "/api/integrations/list", timeout=3) as r:
            data = json.loads(r.read().decode("utf-8"))
    except Exception:
        return set()
    out = set()
    for i in data.get("integrations") or []:
        pid = i.get("id")
        status = i.get("status")
        if isinstance(pid, str) and isinstance(status, str) and status != "not_configured":
            out.add(pid)
    return out
...
def _install_dep_check(spec):
    ...
    Studio can resolve locally, AND against which of those providers
    have credentials configured in the vault. Prints a WARN panel when
    anything is missing but never blocks the install — the user can
    install missing modules + configure missing credentials later, and
    discovery is what matters at install time."""
    want_actions, want_providers = _install_collect_spec_deps(spec)
    if not want_actions and not want_providers:
        return
    have_actions, have_providers, configured_providers, studio_ok = _install_local_action_set()
    if not studio_ok:
        return  # Studio-off is a supported state; skip check silently
    _install_dep_check_report(want_actions, want_providers, have_actions,
                              have_providers, configured_providers)


def _install_dep_check_report(want_actions, want_providers, have_actions,
                              have_providers, configured_providers):
    """Format missing-deps as a friendly WARN panel. Three finding classes:
      · action_id absent    → publisher module isn't installed
      · provider absent     → provider not registered at all (rare)
      · provider present but no vault creds → user needs to configure
        the integration in Studio → Integrations before Plan/Run works."""
    ...
    needs_creds = sorted(p for p in want_providers
                         if p in have_providers and p not in configured_providers
                         and p not in {"http", "transform"})
    if not missing_actions and not missing_providers and not needs_creds:
        return
    lines = _install_dep_check_lines(missing_actions, missing_providers, needs_creds)
    print(panel(lines, title="RAILCALL · market · install · dep check", color="amber"))


def _install_dep_check_lines(missing_actions, missing_providers, needs_creds):
    """Compose the WARN panel body. Extracted so _install_dep_check_report
    stays under 20 lines and the copy is easy to iterate."""
    lines = [c("This workflow references pieces your local Studio can't fully use yet:", "amber"), ""]
    ...
    for p in needs_creds:
        lines.append(c(f"  · provider={p} needs credentials  (configure in Studio → Integrations → {p})", "slate"))
    lines.append("")
    lines.append(c("The workflow will install, but Plan/Run will fail on those nodes until", "dim"))
    lines.append(c("the gaps above are closed. Browse modules: https://railcall.ai/marketplace", "dim"))
    return lines
